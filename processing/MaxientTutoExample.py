...
# Load the datasets
background_data = pd.read_csv('background.csv')
presence_data = pd.read_csv('bradypus_swd.csv')

# Rename columns for consistency
background_data = background_data.rename(columns={'x': 'longitude', 'y': 'latitude'})
presence_data = presence_data.rename(columns={'dd long': 'longitude', 'dd lat': 'latitude'})
...
gdf_test = gpd.GeoDataFrame(test_data_with_coords, geometry=gpd.points_from_xy(test_data_with_coords['longitude'], test_data_with_coords['latitude']))

fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(10, 10))

gdf_test.plot(ax=axes, column=predicted_probabilities, cmap='RdYlBu_r', legend=True, markersize=10)
...
plt.savefig('maxent_all_data.pdf', format='pdf')
plt.tight_layout()
plt.show()
