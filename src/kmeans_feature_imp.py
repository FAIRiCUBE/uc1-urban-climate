...
class KMeansInterp(KMeans):
    ...
    def get_feature_imp_wcss_min(self):
        ...
        for label, centroid in zip(range(labels), sorted_centroid_features_idx):
            ...
            cluster_feature_weights[label] = [ordered_cluster_features,ordered_cluster_feature_weights]
    ...
    def get_feature_imp_unsup2sup(self, X):
        ...
        for label in range(self.n_clusters):
            binary_enc = np.vectorize(
                lambda x: 1 if x == label else 0)(self.labels_)
            clf = RandomForestClassifier()
            clf.fit(X, binary_enc)

            sorted_feature_weight_idxes = np.argsort(
