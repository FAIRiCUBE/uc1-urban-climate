...
class KMeansInterp(KMeans):
    """
    source: https://github.com/YousefGh/kmeans-feature-importance/blob/kmeans-feature-importance-v01/kmeans_interp/kmeans_feature_imp.py
    kmeans_interp is a wrapper around sklearn.cluster.KMeans which adds the property feature_importances_ 
    that will act as a cluster-based feature weighting technique. 
    Features are weighted using either of the two methods: wcss_min or unsup2sup.

    """

    def __init__(
        self, ordered_feature_names, feature_importance_method="wcss_min", **kwargs
    ):
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        if self.feature_importance_method == "wcss_min":
            ...
            raise Exception(
                f" {self.feature_importance_method}"
    ...
    def get_feature_imp_wcss_min(self):
        labels = self.n_clusters  # type: ignore
        ...
        for label, centroid in zip(range(labels), sorted_centroid_features_idx):
            ...
            ordered_cluster_features = [
                self.ordered_feature_names[feature] for feature in centroid
            ]
    ...
    def get_feature_imp_unsup2sup(self, X):
        try:
            ...
            print(IE.__class__.__name__ + ": " + IE.message)  # type: ignore
            raise Exception(
        ...
        cluster_feature_weights = {}
        for label in range(self.n_clusters):  # type: ignore
